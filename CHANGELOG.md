...
- **`extraRepoUrls` / `extraRepoTags` / `extraRepoNames` parameters and `install.ps1` extension point** (fixes #21): new `string[]` parameters (default `[]`) plus three new `install.ps1` parameters (`-ExtraRepoUrls`, `-ExtraRepoTags`, `-ExtraRepoNames`) that let downstream solution accelerators clone arbitrary Git repositories onto the jumpbox under `C:\github\<name>` during the CSE bootstrap, without forking `install.ps1` or repurposing the upstream `manifest.components` array (which is read **after** the upstream clone and therefore not reachable from a consumer's overlay). Tags default to `main`; names default to the repo URL basename without `.git`. The arrays are positional and forwarded to the CSE `commandToExecute` as comma-separated strings via `join(...)`. Existing `manifest.components` behavior is preserved (purely additive). Unblocks the documented "single jumpbox entry point" pattern for private-network data-plane post-provisioning (e.g. Cosmos seeding, AI Search index creation, sample data loading) in consumers like GPT-RAG and `Azure/live-voice-practice`.
